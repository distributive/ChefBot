...
-- |
-- Module      : ChefBot
-- Description : The main runner for the ChefBot Discord bot.
-- License     : MIT
-- Maintainer  : github.com/distributive
-- Stability   : experimental
-- Portability : POSIX
--
-- This module contains the main runner for ChefBot. If you're just looking to
-- run the bot with existing plugins, importing this and your favourite plugins
-- from "ChefBot.Plugins".
module ChefBot
  ( runChefBot,
  )
where
...
-- bot close.
runChefBot :: VersionInfo -> Text -> Text -> FilePath -> [CompiledPlugin] -> IO ()
runChefBot vinfo dToken prefix dbpath plugins =
  do
    ...
    cacheMVar <- newMVar (TCache M.empty vinfo) :: IO (MVar ChefBotCache)
    userFacingError <-
      runDiscord $
        def
          { discordToken = dToken,
      ...
      UpdateStatusOpts
        { updateStatusOptsSince = Nothing,
          updateStatusOptsGame =
            Just
              ( Activity
                  { activityName = "cooking - Try $recipe",
