...
-- | ChefBotEmbedRequest is a request object that mimics various bits of the discord api, just so we can add colours.
...
-- If you suffer from nightmares, don't look in 'ChefBot.Handler.Embed'. Nothing good lives there.
...
data ChefBotEmbedRequest a where ChefBotEmbedRequest :: ChannelId -> Text -> Embed -> ChefBotEmbedRequest Message

instance Request (ChefBotEmbedRequest a) where
  jsonRequest = createEmbedJson
  majorRoute = embedMajorRoute

embedMajorRoute :: ChefBotEmbedRequest a -> String
embedMajorRoute (ChefBotEmbedRequest chan _ _) = "msg " <> show chan

createEmbedJson :: ChefBotEmbedRequest a -> JsonRequest
createEmbedJson (ChefBotEmbedRequest chan msg embed) =
