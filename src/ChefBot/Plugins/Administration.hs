-- |
-- Module      : ChefBot.Plugins.Administration
...
module ChefBot.Plugins.Administration (administrationPlugin) where
...
version = Command "version" noCommand []
  where
    ...
    formatVersions vi = "ChefBot version " <> (pack $ showVersion $ procVersion vi) <> "\nGit Hash: `" <> (gitHash vi) <> "`"
