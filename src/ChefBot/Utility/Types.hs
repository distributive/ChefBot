...
module ChefBot.Utility.Types where
...
--
-- "ChefBot.Plugin.Discord" provides some helper functions for
-- running Discord operations without excessive use of @lift@.
type EnvDatabaseDiscord d = ReaderT d (ReaderT (MVar ChefBotCache) (SqlPersistT DiscordHandler))
...
liftCache :: ReaderT (MVar ChefBotCache) (SqlPersistT DiscordHandler) a -> ReaderT d (ReaderT (MVar ChefBotCache) (SqlPersistT DiscordHandler)) a
liftCache = lift

liftSql :: SqlPersistT DiscordHandler a -> ReaderT d (ReaderT (MVar ChefBotCache) (SqlPersistT DiscordHandler)) a
liftSql = lift . lift

liftDiscord :: DiscordHandler a -> ReaderT d (ReaderT (MVar ChefBotCache) (SqlPersistT DiscordHandler)) a
...
-- "ChefBot.Plugins.Reminder") that has a command for issuing
...
-- not exported by "ChefBot.Plugin") as this structure could change.
data EnvPlugin d = Pl
  { pluginName :: Text,
    startUp :: StartUp d,
