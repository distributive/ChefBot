-- |
-- Module      : ChefBot.Handler
-- Description : The event handler and cron runner for ChefBot.
-- License     : MIT
-- Maintainer  : github.com/distributive
-- Stability   : experimental
-- Portability : POSIX
--
-- This module provides most of the functionality in "ChefBot". This
...
import ChefBot.Internal.Handler.Event
  ( parseMessageChange,
    parseOther,
    parseReactionAdd,
    parseReactionDel,
...
runCron ::
  Pool SqlBackend ->
  CompiledCronJob ->
  ReaderT (MVar ChefBotCache) DiscordHandler ThreadId
runCron pool (CCronJob delay fn) = do
  cache <- ask
  lift . forkIO $ withDelay cache
  where
    withDelay :: MVar ChefBotCache -> DiscordHandler ()
    withDelay cache = do
